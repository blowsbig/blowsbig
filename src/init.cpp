...
#include "config/blowsbig-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "blowsbig.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "blowsbigd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 11338, 11339));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, blowsbig, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-logblowsbig", strprintf(_("Include IP addresses in debug output (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all BSG specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:11338"));
    ...
    strUsage += HelpMessageOpt("-anonymizeblowsbigamount=<n>", strprintf(_("Keep N BSG anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 11338, 11339));
...
/** Sanity checks
 *  Ensure that BSG is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    fLogIPs = GetBoolArg("-logblowsbig", false);
...
#endif
    // Make sure only a single BSG process is using the data directory.
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeBSGAmount = GetArg("-anonymizeblowsbigamount", 0);
    ...
    LogPrintf("Anonymize BSG Amount %d\n", nAnonymizeBSGAmount);
