...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total blowsbig balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized blowsbig balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in blowsbig/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in blowsbig/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"blowsbigaddress\"\n"
            "\nReturn information about the given blowsbig address.\n"
            "\nArguments:\n"
            "1. \"blowsbigaddress\"     (string, required) The blowsbig address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"blowsbigaddress\", (string) The blowsbig address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BSG address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are blowsbig addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) blowsbig address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"blowsbigaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"blowsbigaddress\"  (string, required) The blowsbig address to use for the signature.\n"
