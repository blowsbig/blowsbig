...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BSG address (e.g. %1)").arg("FHoujpfmyWqr69iWhv8D5kpenvSYwG1MGm"));
#endif
...
{
    // return if URI is not valid or is no BSG: URI
...
{
    // Convert blowsbig:// to blowsbig:
    //
    //    Cannot handle this later, because blowsbig:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open blowsbig.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BSG.lnk";
}
...
{
    return GetAutostartDir() / "blowsbig.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a blowsbig.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the blowsbig app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add blowsbig app to startup item list
